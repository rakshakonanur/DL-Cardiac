    ...
    def run(self):
        for case in self.cases:
            ...
            run_simulation_full.main(mode = self.mode, case = case, datadir=self.data_dir, resultsdir = self.results_dir,
                                     PLV = self.PLV, PRV = self.PRV, TA = self.Ta, N = self.N)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--PLV", type=float, default=15.0, help="Left ventricular pressure")
    parser.add_argument("--PRV", type=float, default=3.0, help="Right ventricular pressure")
    parser.add_argument("--Ta", type=float, default=120.0, help="Active stress time constant")
    parser.add_argument("--N", type=int, default=200, help="Number of time steps for simulation")
